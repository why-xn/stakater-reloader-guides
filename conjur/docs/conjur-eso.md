...
1. Check Conjur URL is correct and reachable:
   ```bash
   kubectl run test --rm -it --image=curlimages/curl -- \
     curl -sk https://conjur-conjur-oss.conjur.svc.cluster.local/health
   ```

2. For API Key auth - verify credentials secret exists:
   ```bash
   kubectl get secret conjur-credentials -n eso-test-app
   ```

3. For JWT auth - verify ServiceAccount exists:
   ```bash
   kubectl get serviceaccount eso-jwt-sa -n eso-jwt-test
   ```
...
1. Check ExternalSecret status:
   ```bash
   kubectl describe externalsecret app-secrets -n <NAMESPACE>
   ```

2. Verify Conjur host has permissions:
   ```bash
...
3. Test secret retrieval manually:
   ```bash
   conjur variable get -i secrets/password
   ```

## JWT Authentication Failures

1. Verify JWT authenticator variables are set:
   ```bash
   conjur variable get -i conjur/authn-jwt/dev/issuer
   ```

2. Check the host identity matches the ServiceAccount:
   ```bash
   # The host ID should be: jwt-apps/system:serviceaccount:<namespace>:<sa-name>
   conjur list --kind=host
   ```

3. Verify the audience matches:
   ```bash
...
2. Verify secret has the Reloader annotation:
   ```bash
   kubectl get secret app-secrets -n <NAMESPACE> -o jsonpath='{.metadata.annotations}'
   ```
...
```bash
# Rotate the host's API key in Conjur
conjur host rotate-api-key -i apps/external-secrets

# Update the K8s secret with the new API key
kubectl delete secret conjur-credentials -n eso-test-app
kubectl create secret generic conjur-credentials -n eso-test-app \
  --from-literal=hostId="host/apps/external-secrets" \
